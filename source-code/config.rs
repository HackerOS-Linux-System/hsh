...
# Polecenia całkowicie zablokowane (bez możliwości potwierdzenia) — rozdzielone przecinkami.
# deny_commands = "nc, telnet, ncat"
...
pub fn load_shell_config() -> HkConfig {
    ...
    check_config_integrity(&config_path);
...
/// Ostrzega, jeśli zawartość ~/.hshrc zmieniła się od ostatniego uruchomienia hsh
/// w sposób inny niż przez samo hsh (np. przez inny proces/edytor uruchomiony poza sesją,
/// albo złośliwą modyfikację). Nie blokuje startu — tylko informuje.
fn check_config_integrity(path: &str) {
    ...
    let current = format!("{:016x}", hasher.finish());

    let home = env::var("HOME").unwrap_or_default();
    let checksum_path = format!("{}/.hsh-config-checksum", home);

    if let Ok(stored) = fs::read_to_string(&checksum_path) {
        let stored = stored.trim();
        if !stored.is_empty() && stored != current {
            eprintln!(
                "\x1b[1;31m[hsh] UWAGA:\x1b[0m plik {} zmienił się od ostatniej sesji (inna suma kontrolna).",
                path
            );
            eprintln!(
                "\x1b[38;5;244m       Jeśli to Twoja zmiana (np. przez hsh-settings lub edytor) — zignoruj.\x1b[0m"
            );
            eprintln!(
                "\x1b[38;5;244m       Jeśli nie edytowałeś tego pliku — sprawdź jego zawartość przed dalszą pracą.\x1b[0m"
            );
        }
    }

    let _ = fs::write(&checksum_path, current);
}
...
/// Pobierz opcje bezpieczeństwa z sekcji [safety]
pub fn get_safety_options(config: &HkConfig) -> HashMap<String, String> {
    config
    .get("safety")
    .and_then(|v| v.as_map().ok())
    .map(|m| {
...
/// Pobierz listę całkowicie zablokowanych komend ([safety] deny_commands = "a, b, c").
/// W przeciwieństwie do `check_dangerous`, dopasowanie tutaj nie da się obejść
/// potwierdzeniem — to twarda polityka administratora.
pub fn get_deny_commands(config: &HkConfig) -> Vec<String> {
    get_safety_options(config)
        .get("deny_commands")
        .map(|raw| {
            raw.split(',')
