...
async fn main() -> rustyline::Result<()> {
    ...
    let start_restricted = args.contains(&"--restricted".to_string())
    ...
    if start_restricted {
        ...
        eprintln!("hsh: uruchomiono w trybie restricted (--restricted)");
    }

    // Ostrzeż o klasycznym wektorze ataku: '.' lub katalogi zapisywalne przez
    // wszystkich w PATH mogłyby posłużyć do podstawienia złośliwego binarnego pliku.
    if let Ok(path_var) = env::var("PATH") {
        for warning in security::check_path_hijack_risks(&path_var) {
            eprintln!("\x1b[1;33m[hsh] Ostrzeżenie bezpieczeństwa:\x1b[0m {}", warning);
        }
    }
    ...
    loop {
        ...
        // ── Helper state ─────────────────────────────────────────────────────
        {
            ...
            .and_then(|last| {
                ...
                smart_hints
                .suggest_next_with_context(&last, &cwd)
        ...
        match rl.readline(&prompt) {
            Ok(line) => {
                ...
                // ── Czy user właśnie zaakceptował wcześniej podpowiedzianą poprawkę? ──
                if let Some((typo, correct)) = pending_correction.take() {
                    let this_first = trimmed.split_whitespace().next().unwrap_or("");
                    if this_first == correct {
                        if let Some((t, c)) = smart_hints.record_correction(&typo, &correct) {
                            eprintln!(
                                "  \x1b[38;5;220m💡 Widzę, że kilka razy poprawiłeś '{}' na '{}'. Dodać na stałe: \x1b[1malias {}={}\x1b[0m\x1b[38;5;220m?\x1b[0m",
                                t, c, t, c
                            );
                        }
                    }
                ...
                // Dodaj do historii rustyline (dla Ctrl+R) — zredagowane, żeby
                // sekrety nie trafiły na dysk w ~/.hsh_history_rl w postaci jawnej.
                rl.add_history_entry(security::redact_secrets(&line)).ok();

                let prev_cmd = shell_history.last_command().unwrap_or_default();
                shell_history.add(trimmed);
                smart_hints.record(&prev_cmd, trimmed);
                smart_hints.record_with_context(
                    &prev_cmd, trimmed, &env::current_dir().unwrap_or_default(),
                );
                ...
                if last_exit_code == 127 {
                    ...
                    if let Some(suggestion) =
                        smart_hints.spellcheck(first_word, &path_cache.commands)
                        {
                            ...
                            pending_correction = Some((first_word.to_string(), suggestion.to_string()));
                        }
                }
            ...
            Err(ReadlineError::Interrupted) => {
                ...
                if let Some(cmd) = vars.traps.get("INT").cloned() {
...
// ─────────────────────────────────────────────────────────────────────────────
// Pomocnicze: uruchamianie komend zarejestrowanych przez builtin `trap`
// ─────────────────────────────────────────────────────────────────────────────

fn run_trap_command(cmd: &str) {
    let _ = std::process::Command::new("sh").arg("-c").arg(cmd).status();
}

fn run_exit_trap(vars: &ShellVars) {
    if let Some(cmd) = vars.traps.get("EXIT") {
        run_trap_command(cmd);
    }
}
...
fn apply_shell_options(vars: &mut ShellVars, config: &hk_parser::HkConfig) {
    ...
    // [safety] confirm_dangerous = false  →  wyłącza pytanie o potwierdzenie
    // niebezpiecznych komend (podświetlanie na czerwono w linii pozostaje aktywne).
    let safety = config::get_safety_options(config);
    if let Some(v) = safety.get("confirm_dangerous") {
        vars.confirm_dangerous_enabled = v != "false";
    }
    vars.denied_commands = config::get_deny_commands(config);
}
