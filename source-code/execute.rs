...
async fn run_single(
    ...
    // 6. User-defined functions
    {
        ...
        if let Some(fname) = parts.first() {
            if functions.contains(fname) {
                ...
                // Nowy zakres `local` na czas wykonania funkcji — zmienne ustawione
                // przez builtin `local` znikają po jej zakończeniu (patrz vars.rs).
                vars.push_scope();
    ...
    // 8. Alias expansion
    let rest = expand_alias(&rest, aliases, vars);

    // 9. Auto-sudo
    let rest = check_auto_sudo(&rest);

    // 9b. Twarda lista zablokowanych komend ([safety] deny_commands) — bez potwierdzenia
    if let Some(name) = crate::security::is_denied_command(&rest, &vars.denied_commands) {
        eprintln!("hsh: {}: polecenie zablokowane przez politykę bezpieczeństwa (deny_commands)", name);
        return Ok(126);
    }

    // 9c. Naruszenia trybu restricted (odpowiednik rbash)
    if vars.restricted {
        if let Some(reason) = crate::security::restricted_violation(&rest) {
            eprintln!("hsh: restricted: {}", reason);
            return Ok(1);
        }
    }

    // 10. Dangerous command guard (można wyłączyć: [safety] confirm_dangerous = false w ~/.hshrc)
    if !dry_run && vars.confirm_dangerous_enabled && !confirm_dangerous(&rest) {
        println!("Command aborted.");
        return Ok(1);
    }

    // 10b. Audit log — łańcuchowy zapis wykonanej komendy (tamper-evident)
    if !dry_run {
        crate::security::audit_log(&rest);
    }
...
fn expand_alias(input: &str, aliases: &HashMap<String, String>, vars: &ShellVars) -> String {
    let parts: Vec<String> = shlex::split(input).unwrap_or_default();
    if let Some(first) = parts.first() {
        let resolved = if vars.disabled_aliases.contains(first.as_str()) {
            None
        } else if let Some(v) = vars.runtime_aliases.get(first.as_str()) {
            Some(v)
        } else {
            aliases.get(first.as_str())
        };
